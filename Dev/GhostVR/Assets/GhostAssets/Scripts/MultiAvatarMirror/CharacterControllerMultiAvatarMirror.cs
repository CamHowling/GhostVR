...
public class CharacterControllerMultiAvatarMirror : CharacterControllerInterface
{
    private XRRig rig;
    ...
    protected override void AimScaling(bool isRightHand)
    {
        ...
        if (hand.position != (target.localPosition + transform.position))
        {
            ...
            Vector3 aimLenght = (target.localPosition + transform.position) - UArm.position;
            LArm.position = UArm.position + aimLenght / 2;
            // leftULArm.localScale = scaleIKDirection;
            hand.position = (target.localPosition + transform.position);
        }
    }
    ...
    protected override void calculateBodyPose()
    {
        ...
        if (diffPosition != Vector3.zero)
        {
            ...
            if (MovementSpeed <= 0)
                MovementSpeed = 1;
            Avatar.Move(diffPosition * MovementSpeed);
